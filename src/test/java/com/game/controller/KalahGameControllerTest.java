...
public class KalahGameControllerTest {
	...
	public void testPlay() throws Exception {
		...
		final MockHttpServletRequestBuilder playGameRequest = MockMvcRequestBuilders
				.put("/games/" + game.getGameId() + "/pits/1");
	...
	@Test
	public void testJoinWithZeroStoneMovement() throws Exception {
		KalahGame game = this.service.createOrGetWaitingGame();
		Player firstPlayer = new Player("1");
		this.service.joinPlayer(game, firstPlayer);
		Player secondPlayer = new Player("2");
		this.service.joinPlayer(game, secondPlayer);
		this.service.makeMove(firstPlayer, game.getGameId(), 1);
		Assert.assertTrue("above instructions successfully executed", true);
		try {
			this.service.makeMove(firstPlayer, game.getGameId(), 1);
		}catch(ApplicationException e) {
			Assert.assertEquals(e.getApplicationCode().code(), ErrorCode.ZERO_STONE_PIT_MOVE_NOT_ALLOWED.code());
			Assert.assertEquals(e.getApplicationCode().message(), ErrorCode.ZERO_STONE_PIT_MOVE_NOT_ALLOWED.message());
		}
	}

	@Test
	public void testPlayWithFinalPit_NotKalah() throws Exception {
		KalahGame game = this.service.createOrGetWaitingGame();
		Player firstPlayer = new Player("1");
		this.service.joinPlayer(game, firstPlayer);
		Player secondPlayer = new Player("2");
		this.service.joinPlayer(game, secondPlayer);
		this.service.makeMove(firstPlayer, game.getGameId(), 3);
		Assert.assertTrue("above instructions successfully executed", true);
	}

	@Test
	public void testJoinWithKalahMovement() throws Exception {
		KalahGame game = this.service.createOrGetWaitingGame();
		Player firstPlayer = new Player("1");
		this.service.joinPlayer(game, firstPlayer);
		Player secondPlayer = new Player("2");
		this.service.joinPlayer(game, secondPlayer);
		try {
			this.service.makeMove(firstPlayer, game.getGameId(), 7);
		}catch(ApplicationException e) {
			Assert.assertEquals(e.getMessage(),"Pit is Kalah. Move is not allowed");
			Assert.assertEquals(e.getApplicationCode().code(),"400");
